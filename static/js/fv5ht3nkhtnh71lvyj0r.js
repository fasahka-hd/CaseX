const INITIAL_ROUTE = window.CaseXCore.parseRoute();
const PATH_LANGUAGE = INITIAL_ROUTE.locale.toUpperCase();

const S = {
  page: INITIAL_ROUTE.page, me: null, inventory: [], catalog: [], cases: [], drops: [], online: 0,
  tab: 'inventory', turbo: false,
  upgrade: { from: null, to: null, chance: null, boost: 30, addBalance: 0, spinning: false, result: null, pointerAngle: 0, targetSearch: '', targetMin: '', targetMax: '', targetPage: 1 },
  caseFlow: { opening: null, activeCase: INITIAL_ROUTE.page === 'case' ? INITIAL_ROUTE.id : null, rouletteItems: [], result: null, reveal: false, dropLock: 0, loading: false },
  ...
  notifications: [], unreadNotifications: 0, cookieConsent: null, realtimeStatus: 'DISCONNECTED', paymentsMode: 'disabled'
};
...
function flagIcon(code) {
  ...
  return `<span class="lang-flag"><img src="/chunks/flags/${cc}.svg" alt="${esc(code)}" loading="lazy"></span>`;
}
...
const image = (src, alt = '', local = '') => src
  ? `<img src="${esc(bust(src))}" alt="${esc(alt)}" loading="lazy" decoding="async"${local ? ` data-fb="${esc(bust(local))}"` : ''}>`
  : '<span class="image-placeholder" aria-hidden="true"></span>';
const avatarImage = user => user?.avatar
  ? `<img src="${esc(user.avatar)}" data-fb="/chunks/logo.svg" alt="${esc(user.name || 'Профиль')}" referrerpolicy="no-referrer">`
...
async function api(url, options = {}) {
  try {
    return await window.CaseXCore.fetchJson(url, { ...options, csrfToken: S.me?.csrfToken || '' });
  } catch (error) {
    if (error.status === 401 && S.me) {
...
const stateChannel = typeof BroadcastChannel === 'function' ? new BroadcastChannel('casex-state') : null;
let remoteRefreshTimer = null;
function announceStateChange(scope) {
  stateChannel?.postMessage({ scope, at: Date.now() });
  try { localStorage.setItem('casex:state-change', JSON.stringify({ scope, at: Date.now() })); } catch {}
}
async function operationApi(scope, entity, url, options = {}) {
  const id = window.CaseXCore.operationId(scope, entity);
  try {
    const result = await api(url, { ...options, idempotencyKey: id });
    window.CaseXCore.completeOperation(scope, entity, id);
    announceStateChange(scope);
    return result;
  } catch (error) {
    if (!['REQUEST_TIMEOUT', 'NETWORK_ERROR', 'OPERATION_IN_PROGRESS'].includes(error.code)) {
      window.CaseXCore.completeOperation(scope, entity, id);
    }
    throw error;
  }
...
function scheduleRemoteRefresh() {
  if (!S.me?.authenticated || document.visibilityState === 'hidden') return;
  clearTimeout(remoteRefreshTimer);
  remoteRefreshTimer = setTimeout(() => refreshAccount().then(render).catch(() => {}), 250);
}
stateChannel?.addEventListener('message', scheduleRemoteRefresh);
window.addEventListener('storage', event => {
  if (event.key === 'casex:state-change') scheduleRemoteRefresh();
});
...
const pageDataState = { catalog: false, cases: false };
const pageDataPromises = new Map();
async function ensurePageData(page) {
  const tasks = [];
  if (page === 'upgrade' && !pageDataState.catalog) {
    tasks.push(['catalog', '/api/catalog']);
  }
  if ((page === 'cases' || page === 'case') && !pageDataState.cases) {
    tasks.push(['cases', '/api/cases']);
  }
  await Promise.all(tasks.map(async ([key, endpoint]) => {
    if (pageDataPromises.has(key)) return pageDataPromises.get(key);
    const pending = api(endpoint).then(data => {
      if (key === 'catalog') S.catalog = shuffleCatalog(Array.isArray(data) ? data : []);
      if (key === 'cases') S.cases = Array.isArray(data?.cases) ? data.cases : [];
      pageDataState[key] = true;
    }).catch(() => {}).finally(() => pageDataPromises.delete(key));
    pageDataPromises.set(key, pending);
    return pending;
  }));
  if (page === 'case' && S.caseFlow.activeCase) await loadCaseDetail(S.caseFlow.activeCase);
}
async function loadCaseDetail(caseId) {
  const existing = S.cases.find(item => item.id === caseId);
  if (existing?.contents?.length) return existing;
  S.caseFlow.loading = true;
  render();
  try {
    const data = await api(`/api/cases/${encodeURIComponent(caseId)}`);
    const detail = data?.case;
    if (!detail) return null;
    const index = S.cases.findIndex(item => item.id === caseId);
    if (index >= 0) S.cases[index] = detail;
    else S.cases.push(detail);
    return detail;
  } catch (error) {
    toast(error.message, 'error');
    return null;
  } finally {
    S.caseFlow.loading = false;
    render();
  }
}
...
async function boot() {
  ...
  const critical = await Promise.allSettled([api('/api/config'), api('/api/me')]);
  const [config, me] = window.CaseXCore.resultValues(critical, [{}, { authenticated: false }]);
  S.brand = config.brand || S.brand;
  S.telegram = config.telegram || S.telegram;
  S.supportEmail = config.supportEmail || S.supportEmail;
  S.marketingEmail = config.marketingEmail || S.marketingEmail;
  S.siteBanner = config.banner || null;
  S.paymentsMode = config.paymentsMode || 'disabled';
  S.me = me;
  const common = await Promise.allSettled([api('/api/live-drops'), api('/api/online'), api('/api/stats')]);
  const [drops, online, stats] = window.CaseXCore.resultValues(common, [[], { online: 0 }, S.globalStats]);
  S.drops = Array.isArray(drops) ? drops : [];
  S.online = Number(online?.online || 0);
  S.globalStats = stats || S.globalStats;
  await ensurePageData(INITIAL_ROUTE.page);
  if (me.authenticated) {
    const accountData = await Promise.allSettled([api('/api/inventory'), api('/api/profile'), api('/api/cx-weekly')]);
    const [inventory, profile, weekly] = window.CaseXCore.resultValues(accountData, [{ items: [], feed: [] }, null, null]);
    S.inventory = inventory?.items || [];
    S.profile = profile;
    S.cxWeekly = weekly;
  }
  if (INITIAL_ROUTE.page === 'public-profile' && INITIAL_ROUTE.id) await openPublicProfile(INITIAL_ROUTE.id, false);
  else render();
  listen();
  if (critical.some(result => result.status === 'rejected')) toast('Часть основных данных временно недоступна', 'error');
}

let realtimeClient = null;
function listen() {
  realtimeClient?.close();
  realtimeClient = new window.CaseXCore.RealtimeClient('/api/events', {
    online(data) {
      S.online = Number(data?.online || 0);
      updateOnline();
    },
    drop(drop) {
      ...
      if (drop.source === 'reward' && S.me?.authenticated) {
        api('/api/cx-weekly').then(weekly => { S.cxWeekly = weekly; render(); }).catch(() => {});
    ...
    notify(notification) {
      ...
      if (notification.audience === 'guests' && S.me?.authenticated) return;
      if (notification.audience === 'authenticated' && !S.me?.authenticated) return;
      if (notification.audience === 'staff' && !['admin', 'support'].includes(S.me?.user?.role)) return;
      if (S.notifications.some(item => item.id === notification.id)) return;
      S.notifications.unshift(notification);
      S.notifications = S.notifications.slice(0, 20);
      S.unreadNotifications = Number(S.unreadNotifications || 0) + 1;
      render();
    }
  }, status => {
    S.realtimeStatus = status;
    document.body.dataset.realtime = status.toLowerCase();
  });
  realtimeClient.connect();
  api('/api/notifications').then(data => {
    const list = Array.isArray(data?.notifications) ? data.notifications : [];
    const filtered = list.filter(notification => {
      if (notification.audience === 'guests' && S.me?.authenticated) return false;
      if (notification.audience === 'authenticated' && !S.me?.authenticated) return false;
      if (notification.audience === 'staff' && !['admin', 'support'].includes(S.me?.user?.role)) return false;
      return true;
    });
    S.notifications = filtered.slice(0, 20);
    const seen = notificationSeenIds();
    S.unreadNotifications = filtered.reduce((total, notification) => total + (seen.has(notification.id) ? 0 : 1), 0);
    render();
  }).catch(() => {});
  ...
  window.addEventListener('beforeunload', () => realtimeClient?.close(), { once: true });
}
...
function header() {
  ...
  return `<header class="top">
    ...
    <div class="actions">
      <a class="telegram" href="${esc(safeHref(S.telegram))}" target="_blank" rel="noopener">${telegramIcon()}</a>
...
function upgradePage() {
  ...
  const baseValue = (S.upgrade.from ? Number(S.upgrade.from.priceCents) : 0) + S.upgrade.addBalance;
  const minimumPrice = S.upgrade.from ? boostMinimumPrice(baseValue, S.upgrade.boost) : 0;
  const query = S.upgrade.targetSearch.trim().toLowerCase();

  const userMin = S.upgrade.targetMin === '' ? 0 : Number(S.upgrade.targetMin) * 100;
  ...
  const chance = clampChance(S.upgrade.chance);
  const chanceText = formatChance(S.upgrade.chance);
  const boostOptions = [['30%', 30], ['50%', 50], ['75%', 75], ['x2', 200], ['x5', 500], ['x10', 1000]];
  const boostButtons = boostOptions.map(([label, value]) => `<button class="${S.upgrade.boost === value ? 'active' : ''}" ${S.upgrade.from ? '' : 'disabled'} onclick="setBoost(${value})">${label}</button>`).join('');
  const balanceCents = Number(S.me.user.balanceCents || 0);
  const balanceR = (balanceCents / 100).toFixed(2);
  const addR = (S.upgrade.addBalance / 100).toFixed(2);
  const rangeProgress = balanceCents > 0 ? Math.min(100, S.upgrade.addBalance / balanceCents * 100) : 0;
  ...
  const fromLabel = S.upgrade.from ? `${money(S.upgrade.from.priceCents)}${S.upgrade.addBalance ? ` + ${money(S.upgrade.addBalance)}` : ''}` : '';
  const leftEmpty = '<div class="selected-big empty">Выбранный предмет появится здесь</div>';
  return `<section class="upgrid${S.upgrade.spinning ? ' cx-upgrade-locked' : ''}">
      <div class="source-col">
        ...
        <div class="panel"><div class="title">ВЫБЕРИТЕ <b>&nbsp;ПРЕДМЕТ ДЛЯ ИСПОЛЬЗОВАНИЯ</b></div>
          ${S.upgrade.from ? bigSelected(S.upgrade.from) : leftEmpty}
        </div>
        <div class="add-balance-bar">
          ...
          <div class="add-balance-mid"><div class="ab-value"><b>${money(S.upgrade.addBalance)}</b></div>
      ...
      <div class="wheelbox${S.upgrade.spinning ? ' is-spinning' : ''}">
        <div class="wheelhead"><span>ШАНС АПГРЕЙДА</span><b>${chanceText}</b></div>
        <div class="wheel-stage"><div class="wheel${S.upgrade.spinning ? ' is-spinning' : ''}" style="--chance:${chance}%;--chance-half:${(chance / 2).toFixed(3)}%">
          ...
          <div class="wheelcenter"><strong>${chanceText}</strong><span>${S.upgrade.chance == null ? 'выберите предметы' : 'шанс апгрейда'}</span></div>
        </div><div class="pointer-orbit" style="--angle:${Number(S.upgrade.pointerAngle) || 0}deg" aria-hidden="true"><img class="pointer" src="/chunks/spinner-arrow.webp" alt=""></div></div>
        <button class="upgrade" ${!S.upgrade.from || !S.upgrade.to || S.upgrade.spinning ? 'disabled' : ''} onclick="upgrade()">
          <img class="upgrade-icon" src="/chunks/upgrade.svg" alt="">${S.upgrade.spinning ? 'АПГРЕЙД...' : 'АПГРЕЙД'}
        </button>
        <div class="under">${S.upgrade.from && S.upgrade.to ? `${fromLabel} → ${money(S.upgrade.to.priceCents)}` : S.upgrade.from ? `Цель от ${money(minimumPrice)}` : 'Выберите предмет слева и цель справа'}</div>
      </div>
      <div class="target-col">
        <div class="panel target-preview-panel"><div class="title">ВЫБРАННАЯ <b>&nbsp;ЦЕЛЬ АПГРЕЙДА</b></div>
          ${S.upgrade.to ? bigSelected(S.upgrade.to) : leftEmpty}
        </div>
        <div class="boost-row"><div class="boost-label">УВЕЛИЧИТЬ СТОИМОСТЬ ЦЕЛИ</div><div class="boost-buttons">${boostButtons}</div></div>
      </div>
    </section>
    <section class="lower upgrade-lists${S.upgrade.spinning ? ' cx-upgrade-locked-lists' : ''}">
      ...
      <div class="panel upgrade-list-panel"><div class="upgrade-list-head"><div><img src="/chunks/upgrade.svg" alt=""><b>ВЫБРАТЬ ПРЕДМЕТ</b></div><div class="target-filters"><input id="target-min" value="${esc(S.upgrade.targetMin)}" placeholder="От" inputmode="decimal" oninput="applyTargetFilters()"><input id="target-max" value="${esc(S.upgrade.targetMax)}" placeholder="До" inputmode="decimal" oninput="applyTargetFilters()"><input id="target-search" value="${esc(S.upgrade.targetSearch)}" placeholder="Поиск" oninput="applyTargetFilters()"><button onclick="applyTargetFilters()" aria-label="Поиск">⌕</button></div></div><div class="upgrade-list-body">${pageTargets.length ? `<div class="grid upgrade-items-grid target-items-grid">${pageTargets.map(item => skinCard(item, { button: true, onclick: `choose('${item.catalogId}','to')` })).join('')}</div>` : '<div class="upgrade-list-empty"><strong>Предметы не найдены</strong><span>Измените фильтры или выберите исходный предмет</span></div>'}${pageCount > 1 ? `<div class="target-pager"><button ${S.upgrade.targetPage <= 1 ? 'disabled' : ''} onclick="setTargetPage(${S.upgrade.targetPage - 1})" aria-label="Предыдущая страница">‹</button><button ${S.upgrade.targetPage >= pageCount ? 'disabled' : ''} onclick="setTargetPage(${S.upgrade.targetPage + 1})" aria-label="Следующая страница">›</button></div>` : ''}</div></div>
    </section>`;
}
...
async function sellSelectedItems() {
  ...
  for (const item of items) {
    try {
      const result = await operationApi('inventory_sell', item.assetid, `/api/inventory/${encodeURIComponent(item.assetid)}/sell`, { method: 'POST' });
      sold++;
  ...
  S.upgrade.from = null; S.upgrade.to = null; S.upgrade.chance = null;
  render();
...
async function cxDoSellAll() {
  try {
    const r = await operationApi('inventory_sell_all', 'all', '/api/cx-sell-all', { method: 'POST' });
...
function inventoryPage() {
  ...
  if (!S.inventory.length) return `<h1 class="page-title">Инвентарь сайта</h1><p class="sub">Здесь хранятся только предметы из кейсов и апгрейдов сайта.
...
function playCaseDrop() {
  ...
  if (S.page !== 'case' || !S.caseFlow.activeCase || S.caseFlow.opening || S.caseFlow.rouletteItems.length) {
    if (fx) fx.remove();
    if (src) src.style.visibility = '';
    if (S.caseFlow.opening || S.caseFlow.rouletteItems.length) caseDropFor = S.caseFlow.activeCase;
    return;
  }
  if (fx) {
...
function caseDetailPage() {
  ...
  const caseData = S.cases.find(item => item.id === S.caseFlow.activeCase);
  if (!caseData) return `<div class="empty"><h2>${S.caseFlow.loading ? 'Загрузка кейса…' : 'Кейс не найден'}</h2></div>`;
  const spinning = S.caseFlow.opening === caseData.id;
  const insufficient = caseData.priceCents > Number(S.me.user.balanceCents);
  const disabled = S.caseFlow.loading || !!S.caseFlow.opening || !caseData.available || insufficient;
  const missingCents = caseData.priceCents - Number(S.me.user.balanceCents);
  const buyButton = !insufficient
    ? `<button class="case-buy-button" ${disabled ? 'disabled' : ''} onclick="openCase('${esc(caseData.id)}')">${S.caseFlow.loading ? 'ЗАГРУЗКА...' : S.caseFlow.opening ? 'ОТКРЫВАЕМ...' : !caseData.available ? 'УЖЕ ОТКРЫТ' : caseData.priceCents ? `КУПИТЬ ЗА ${coinPrice(caseData.priceCents)}` : 'КУПИТЬ БЕСПЛАТНО'}</button>`
  ...
  const buyBlock = S.caseFlow.result ? '' : buyButton;
  const roulette = S.caseFlow.rouletteItems.length ? `<div class="case-roulette"><i class="roulette-pointer"></i><div class="case-roll-track">${S.caseFlow.rouletteItems.map(rouletteCard).join('')}</div></div>` : '';
  const result = S.caseFlow.result && !S.caseFlow.reveal ? `<div class="case-result cx-result-clean"><span>ВЫПАЛО</span>${skinCard(S.caseFlow.result, { className: 'case-result-item' })}<button class="cx-res-up" onclick="sendToUpgrade('${S.caseFlow.result.assetid}')">В АПГРЕЙД</button><button class="cx-res-inv" onclick="openInventory()">В ИНВЕНТАРЬ</button></div>` : '';
  ...
  return `<button class="case-back" onclick="go('cases')">← ВСЕ КЕЙСЫ</button>
    <section class="case-detail ${spinning ? 'cx-case-spinning' : ''}">
      <div class="panel-tools case-tools" style="position:absolute;top:14px;right:14px;z-index:6">
        <button type="button" class="turbo-toggle ${S.turbo ? 'active' : ''}" aria-label="${caseTurboLabel}" aria-pressed="${S.turbo}" title="${caseTurboLabel}" ${S.caseFlow.opening ? 'disabled' : ''} onclick="toggleTurbo()">${turboIcon(S.turbo)}</button>
      </div>
...
async function cxClaimWeekly(catalogId) {
  ...
  if (!approved) return;
  try {
    const r = await operationApi('weekly_claim', `${S.cxWeekly?.expires || 0}:${catalogId}`, '/api/cx-weekly/claim/' + encodeURIComponent(catalogId), { method: 'POST' });
...
function historyCard(item) {
  const status = item.status === 'SOLD' ? 'ПРОДАНО' : item.status === 'CONSUMED' ? 'ИСПОЛЬЗОВАНО' : '';
  const icon = item.icon || item.localIcon || '';
  const img = icon ? `<img src="${esc(bust(icon))}" alt="" loading="lazy">` : '<span class="image-placeholder" aria-hidden="true"></span>';
...
async function openPublicProfile(userId, push = true) {
  ...
  if (!id) return;
  if (S.me?.authenticated && Number(S.me.user.id) === id) {
    if (push) history.pushState({}, '', window.CaseXCore.routePath(S.footerLang, 'profile'));
    S.page = 'profile';
    render();
    return;
  }
  ...
  S.publicProfileError = '';
  S.publicProfileLoading = true;
  S.profileTab = 'items';
  if (push) history.pushState({ profileId: id }, '', window.CaseXCore.routePath(S.footerLang, 'public-profile', id));
  render();
  try { S.publicProfile = await api(`/api/users/${id}/profile`); }
  catch (error) { S.publicProfileError = error.message; }
  finally { S.publicProfileLoading = false; render(); }
}
window.addEventListener('popstate', () => {
  if (S.upgrade.spinning || S.caseFlow.opening) return;
  const route = window.CaseXCore.parseRoute();
  S.footerLang = route.locale.toUpperCase();
  if (route.page === 'public-profile' && route.id) {
    openPublicProfile(route.id, false);
    return;
  }
  S.page = route.page;
  S.caseFlow.activeCase = route.page === 'case' ? route.id : null;
  S.publicProfile = null;
  resetPageFlows(route.page);
  render();
  ensurePageData(route.page).then(render);
});
...
function siteFooter() {
  ...
  return `<footer class="site-footer">
    <div class="footer-mobile-head"><div><img src="/chunks/logo.svg" alt=""><b>${esc(S.brand)}</b></div><a href="${esc(safeHref(S.telegram))}" target="_blank" rel="noopener">${telegramIcon()}</a></div>
...
function siteBanner() {
  ...
  return `<aside class="site-announcement tone-${esc(banner.tone||'info')}"><div><b>${esc(banner.title)}</b><span>${esc(banner.body)}</span></div>${banner.link?`<a href="${esc(safeHref(banner.link))}">Подробнее</a>`:''}</aside>`;
}
...
function openInventory() {
  if (S.upgrade.spinning || S.caseFlow.opening) return toast('Дождитесь завершения операции', 'error');
  S.page = 'profile';
  S.profileTab = 'items';
  history.pushState({}, '', window.CaseXCore.routePath(S.footerLang, 'profile'));
  render();
}
...
function render() {
  ...
  const html = localizeMarkup(translateMarkup(rawHtml));
...
function syncCaseBackdrop() {
  ...
  if (!layer.querySelector('video')) {
    layer.innerHTML = localizeMarkup('<video class="cx-case-backdrop-video" src="/cases/cases/case-bg-video.webm" poster="/cases/case-bg-poster2.webp" autoplay muted loop playsinline disablePictureInPicture></video>');
  }
...
function ensureImages() {
  window.CaseXCore.enhanceImages(document);
}

function upgradeBaseValue() {
  return (S.upgrade.from ? Number(S.upgrade.from.priceCents) : 0) + Number(S.upgrade.addBalance || 0);
}
...
function recalculateChance() {
  ...
  const targetPrice = S.upgrade.to ? Number(S.upgrade.to.priceCents) : 0;
  if (!S.upgrade.from || !S.upgrade.to || !Number.isFinite(targetPrice) || targetPrice <= 0) { S.upgrade.chance = null; return; }

  S.upgrade.chance = clampChance(Math.floor(value / targetPrice * 10000) / 100);
}
...
function upgradeTargets() {
  if (!S.upgrade.from) return [];
  const minimum = boostMinimumPrice(upgradeBaseValue(), S.upgrade.boost);
  return S.catalog.filter(item => item.priceCents >= minimum).sort((a, b) => a.priceCents - b.priceCents);
}
async function choose(id, side) {
  if (S.upgrade.spinning) return;
  ...
  if (side === 'from') {
    S.upgrade.from = item;
    setBoost(S.upgrade.boost, false);
    recalculateChance();
    render();
    return;
  }
...
function setBoost(value, shouldRender = true) {
  if (S.upgrade.spinning) return;
  S.upgrade.boost = Number(value);
  S.upgrade.targetPage = 1;
  const targets = upgradeTargets();
  S.upgrade.to = targets[0] || null;
  recalculateChance();
...
function setAddBalance(value) {
  if (S.upgrade.spinning) return;
  const balanceCents = Number(S.me?.user?.balanceCents || 0);
  S.upgrade.addBalance = Math.round(Math.min(Math.max(Number(value) || 0, 0), balanceCents / 100) * 100);
  S.upgrade.targetPage = 1;
  if (S.upgrade.from && S.upgrade.to) {
    const minimum = boostMinimumPrice(upgradeBaseValue(), S.upgrade.boost);
    if (Number(S.upgrade.to.priceCents) < minimum) {
      const targets = upgradeTargets();
      S.upgrade.to = targets[0] || null;
    }
  }
...
function syncAddBalanceRange() {
  ...
  const stateValue = (S.upgrade.addBalance / 100).toFixed(2);
...
function applyTargetFilters() {
  ...
  S.upgrade.targetMin = document.querySelector('#target-min')?.value?.trim() || '';
  S.upgrade.targetMax = document.querySelector('#target-max')?.value?.trim() || '';
  S.upgrade.targetSearch = document.querySelector('#target-search')?.value?.trim() || '';
  S.upgrade.targetPage = 1;
  render();
  if (activeId) {
...
function setTargetPage(page) {
  ...
  const maxT = S.upgrade.targetMax === '' ? Infinity : Number(S.upgrade.targetMax) * 100;
  ...
  S.upgrade.targetPage = Math.max(1, Math.min(page, count));
  render();
}
function sendToUpgrade(id) {
  ...
  S.page = 'upgrade';
  resetPageFlows('upgrade');
  S.upgrade.from = item;
  setBoost(S.upgrade.boost, false);
  history.pushState({}, '', window.CaseXCore.routePath(S.footerLang, 'upgrade'));
  render();
  ensurePageData('upgrade').then(() => { setBoost(S.upgrade.boost, false); render(); });
}
async function sellItem(id) {
  try {
    const result = await operationApi('inventory_sell', id, `/api/inventory/${encodeURIComponent(id)}/sell`, { method: 'POST' });
    await refreshAccount();
    if (S.upgrade.from && String(S.upgrade.from.assetid) === String(id)) { S.upgrade.from = null; S.upgrade.to = null; S.upgrade.chance = null; }
    render();
...
async function refreshAccount() {
  const requests = [
    ...
    ['profile', '/api/profile'],
    ['drops', '/api/live-drops'],
    ['stats', '/api/stats']
  ];
  if (S.page === 'cases' || S.page === 'case') requests.push(['cases', '/api/cases']);
  if (S.page === 'rewards') requests.push(['weekly', '/api/cx-weekly']);
  const results = await Promise.allSettled(requests.map(([, endpoint]) => api(endpoint)));
  results.forEach((result, index) => {
    if (result.status !== 'fulfilled') return;
    const key = requests[index][0];
    const value = result.value;
    if (key === 'me') S.me = value;
    if (key === 'inventory') {
      S.inventory = value.items || [];
    }
    if (key === 'profile') S.profile = value;
    if (key === 'drops') S.drops = value;
    if (key === 'stats') S.globalStats = value;
    if (key === 'cases') { S.cases = value.cases || []; pageDataState.cases = true; }
    if (key === 'weekly') S.cxWeekly = value;
  });
  const balanceCents = Number(S.me?.user?.balanceCents || 0);
  if (S.upgrade.addBalance > balanceCents) S.upgrade.addBalance = Math.max(0, balanceCents);
  const selected = S.upgrade.from && S.inventory.find(item => String(item.assetid) === String(S.upgrade.from.assetid));
  if (S.upgrade.from && !selected) {
    S.upgrade.from = null;
    S.upgrade.to = null;
    S.upgrade.chance = null;
  }
}

function selectCase(caseId) {
  if (S.upgrade.spinning || S.caseFlow.opening) return toast('Дождитесь завершения операции', 'error');
  S.caseFlow.activeCase = caseId;
  S.caseFlow.result = null;
  S.caseFlow.reveal = false;
  S.caseFlow.rouletteItems = [];
  S.page = 'case';
  caseDropFor = null;
  history.pushState({ caseId }, '', window.CaseXCore.routePath(S.footerLang, 'case', caseId));
  window.scrollTo(0, 0);
  document.documentElement.scrollTop = 0;
  document.body.scrollTop = 0;
  render();
  S.caseFlow.dropLock = Date.now() + 1300;
...
async function openCase(caseId) {
  ...
  if (!caseData) return;
  window.CaseXCore.rememberFocus();
  S.caseFlow.opening = caseId;
  S.caseFlow.result = null;
  S.caseFlow.reveal = false;
  S.caseFlow.rouletteItems = [];
  S.caseFlow.dropLock = 0;
  render();
  const detail = document.querySelector('.case-detail');
  if (detail) detail.classList.add('cx-case-spinning');
  try {
    const result = await operationApi('case_open', caseId, '/api/cases/open', {
    ...
    S.caseFlow.rouletteItems = buildRoulette(caseData, result.item);
    render();
    ...
    S.caseFlow.rouletteItems = [];
    S.caseFlow.result = result.item;
    S.caseFlow.reveal = true;
  } catch (error) {
    S.caseFlow.rouletteItems = [];
    window.CaseXCore.restoreFocus();
    toast(error.message, 'error');
  } finally {
    if (detail) detail.classList.remove('cx-case-spinning');
    S.caseFlow.opening = null;
    render();
  }
}
...
function applyPointerAngle(deg) {
  S.upgrade.pointerAngle = Math.round(deg * 100) / 100;
  const orbit = document.querySelector('.pointer-orbit');
  if (orbit) orbit.style.setProperty('--angle', S.upgrade.pointerAngle + 'deg');
}
...
function startPointerSpin() {
  ...
  let last = t0;
  const step = now => {
    if (!S.upgrade.spinning) { pointerRaf = null; return; }
...
async function upgrade() {
  if (!S.upgrade.from || !S.upgrade.to || S.upgrade.spinning) return;
  const fromSnapshot = { ...S.upgrade.from };
  const toSnapshot = { ...S.upgrade.to };
  const boostSnapshot = S.upgrade.boost;
  const balanceSnapshot = S.upgrade.addBalance;
  window.CaseXCore.rememberFocus();
  S.upgrade.spinning = true;
  startPointerSpin();
  render();
  try {
    const entity = `${fromSnapshot.assetid}:${toSnapshot.catalogId}:${boostSnapshot}:${balanceSnapshot}`;
    const result = await operationApi('upgrade', entity, '/api/upgrade', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fromAssetId: fromSnapshot.assetid, toCatalogId: toSnapshot.catalogId, boostPercent: boostSnapshot, addBalanceCents: balanceSnapshot })
    });
    if (typeof result.balanceCents === 'number' && S.me?.user) {
      S.me.user.balanceCents = result.balanceCents;
    ...
    S.upgrade.chance = clampChance(result.chance);
    const wheelEl = document.querySelector('.wheel');
    if (wheelEl) {
      wheelEl.style.setProperty('--chance', S.upgrade.chance + '%');
      wheelEl.style.setProperty('--chance-half', (S.upgrade.chance / 2).toFixed(3) + '%');
      wheelEl.style.setProperty('--chance-deg', (S.upgrade.chance * 3.6).toFixed(3) + 'deg');
      wheelEl.style.setProperty('--chance-half-deg', (S.upgrade.chance * 1.8).toFixed(3) + 'deg');
    }
    const headEl = document.querySelector('.wheelhead b');
    if (headEl) headEl.textContent = formatChance(S.upgrade.chance);
    const centerEl = document.querySelector('.wheelcenter strong');
    if (centerEl) centerEl.textContent = formatChance(S.upgrade.chance);
    ...
    S.upgrade.to = null;
    S.upgrade.chance = null;
    S.upgrade.addBalance = 0;
  } catch (error) {
    S.upgrade.result = null;
    if (pointerRaf) cancelAnimationFrame(pointerRaf);
    pointerRaf = null;
    pointerCurrent = 0;
    applyPointerAngle(0);
    ...
    S.upgrade.spinning = false;
    render();
    if (S.upgrade.result) {
      setTimeout(() => { if (S.upgrade.result) { S.upgrade.result = null; render(); } }, 8000);
    }
  }
}
...
function caseRevealOverlay() {
  if (!S.caseFlow.reveal || !S.caseFlow.result) return '';
...
function resultOverlay() {
  ...
  return `<div class="result-overlay ${won ? 'win' : 'lose'}" role="dialog" aria-modal="true" aria-label="${won ? 'Апгрейд успешен' : 'Апгрейд не удался'}">
    <div class="result-card">
      ...
      ${resultItemBlock(item)}
      ${resultParticles()}
      <button class="result-close" data-dialog-cancel onclick="closeUpgradeResult()">ПРОДОЛЖИТЬ</button>
    </div>
  </div>`;
}
...
function setFooterLang(code) {
  ...
  const id = S.page === 'case' ? S.caseFlow.activeCase : S.page === 'public-profile' ? S.publicProfile?.user?.id : null;
  location.assign(window.CaseXCore.routePath(normalized, S.page, id));
}
...
function resetPageFlows(nextPage) {
  if (nextPage !== 'case') {
    S.caseFlow.activeCase = null;
    S.caseFlow.rouletteItems = [];
    S.caseFlow.result = null;
    S.caseFlow.reveal = false;
    S.caseFlow.dropLock = 0;
...
function go(page) {
  ...
  setTimeout(() => {
    ...
    S.publicProfile = null;
    resetPageFlows(page);
    history.pushState({}, '', window.CaseXCore.routePath(S.footerLang, page));
    render();
    ensurePageData(page).then(render);
...
function confirmSteamLogin() {
  ...
  location.href = window.CaseXCore.appUrl('/auth/steam');
}
...
function closeSettings() { S.settingsOpen = false; render(); window.CaseXCore.restoreFocus(); }
...
function openPayment() {
  if (S.paymentsMode === 'disabled') return toast('Платёжный провайдер временно недоступен', 'error');
  window.CaseXCore.rememberFocus();
  S.paymentOpen = true;
  render();
}
function closePayment() { S.paymentOpen = false; render(); window.CaseXCore.restoreFocus(); }
...
async function submitPayment() {
  ...
  try {
    const r = await operationApi('payment_simulation', amountCents, '/api/cx-deposit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amountCents }) });
    S.paymentOpen = false;
    S.me.user.balanceCents = r.balanceCents;
    if (S.me?.authenticated) {
...
function closeChat() { S.chat = false; render(); window.CaseXCore.restoreFocus(); }
...
function closeNotifications(immediate = false) {
  ...
  const remove = () => { overlay.remove(); window.CaseXCore.restoreFocus(); window.CaseXCore.syncDialog(document); };
...
async function openNotifications() {
  ...
  try {
    ...
    const markup = `<div class="notifications-overlay" data-notifications-overlay><div class="notifications-panel fixed-notifications" role="dialog" aria-modal="true" aria-label="Уведомления"><div class="notifications-head"><span>Уведомления</span><div class="notifications-count">${S.notifications.length}</div></div><div class="notifications-list">${body}</div><div class="notifications-foot"><button type="button" data-dialog-cancel onclick="closeNotifications()" aria-label="Закрыть"><img src="/chunks/arrowDownIcon.svg" alt=""></button></div></div></div>`;
    document.body.insertAdjacentHTML('beforeend', localizeMarkup(translateMarkup(markup)));
    const overlay = document.querySelector('[data-notifications-overlay]');
    window.CaseXCore.enhanceImages(overlay);
