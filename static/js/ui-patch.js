...
function removeOnlineMarker(profile){profile.querySelectorAll('*').forEach(el=>{if(el.tagName==='IMG')return;const c=String(el.className||'').toLowerCase();const a=String(el.getAttribute?.('aria-label')||'').toLowerCase();if(/online|status|онлайн/.test(c)||/online|онлайн/.test(a))el.remove()})}
function patchHeader(){const actions=document.querySelector('.actions');if(!actions)return;const profile=actions.querySelector('.profile-trigger');if(!profile)return;profile.classList.add('square-avatar');removeOnlineMarker(profile);profile.querySelectorAll('img').forEach(img=>{img.style.width='100%';img.style.height='100%';img.style.objectFit='cover'});actions.querySelectorAll('.notification-trigger').forEach((btn,index)=>{if(index>0||!btn.id)btn.remove()})}
function patch(){injectCss();patchHeader()}
new MutationObserver(patch).observe(document.documentElement,{childList:true,subtree:true});requestAnimationFrame(patch)
})();
