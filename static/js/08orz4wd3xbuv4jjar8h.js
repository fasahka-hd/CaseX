(function () {
  ...
  const state = {
    ...
    csrfToken: '', realtimeStatus: 'DISCONNECTED'
  };
  ...
  async function api(url, options = {}) {
    return window.CaseXCore.fetchJson(url, { ...options, csrfToken: state.csrfToken });
  }

  async function operationApi(scope, entity, url, options = {}) {
    const key = window.CaseXCore.operationId(scope, entity);
    try {
      const result = await api(url, { ...options, idempotencyKey: key });
      window.CaseXCore.completeOperation(scope, entity, key);
      return result;
    } catch (error) {
      if (!['REQUEST_TIMEOUT', 'NETWORK_ERROR', 'OPERATION_IN_PROGRESS'].includes(error.code)) window.CaseXCore.completeOperation(scope, entity, key);
      throw error;
    }
  }
  ...
  function render() {
    ...
    window.CaseXCore.enhanceImages(root);
    window.CaseXCore.syncDialog(root);
  ...
  function bindUserModal(view) {
    ...
    if (balApply) {
      balApply.addEventListener('click', async () => {
        ...
        await run(() => operationApi('admin_balance', `${id}:${amountCents}:${note}`, `/api/admin/users/${id}/balance`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    if (giveApply) {
      giveApply.addEventListener('click', async () => {
        ...
        await run(() => operationApi('admin_give', `${id}:${catalogId}`, `/api/admin/users/${id}/give`, {
  ...
  async function boot() {
    try {
      ...
      if (!me.authenticated) {
        ...
        root.innerHTML = `<div><h2>Требуется вход</h2><p>Авторизуйтесь через Steam на основном сайте.</p><a href="${window.CaseXCore.appUrl('/')}">На главную</a></div>`;
        return;
      }
      state.me = me.user;
      ...
      if (state.role !== 'admin' && state.role !== 'support') {
        root.className = 'admin-denied';
        root.innerHTML = `<div><h2>Доступ запрещён</h2><p>Раздел доступен администраторам и поддержке.</p><a href="${window.CaseXCore.appUrl('/')}">На главную</a></div>`;
        return;
      }
      ...
      const events = new window.CaseXCore.RealtimeClient('/api/events', {
        'support-ticket'(ticket) {
          if (!ticket || typeof ticket !== 'object') return;
          toast(`Новый тикет: ${ticket.userName} · ${TICKET_CATEGORIES[ticket.category] || ticket.category}`, 'ok');
          if (state.tab === 'support') loadSupport();
        }
      }, status => { state.realtimeStatus = status; });
      events.connect();
      ...
      root.className = 'admin-denied';
      root.innerHTML = `<div><h2>Ошибка</h2><p>${esc(error.message)}</p><a href="${window.CaseXCore.appUrl('/')}">На главную</a></div>`;
    }
  }
